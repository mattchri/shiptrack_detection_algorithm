...
;PRINT,'HDF FILE INPUT'
 ;Get MODIS DATA 
 read_mod02,HDF_FILE[0],rad02,str2,str2_long,units2
...
xDev = REFORM( TrackData[tI,0,*] )
yDev = REFORM( TrackData[tI,1,*] )
...
;Plot Histogram of Effective Radius
  ...
  ID1=WHERE(X1 GT 0.,idct1)
  ID2=WHERE(X2 GT 0.,idct2)
  if idct1 gt 2 and idct2 gt 2 then begin
  X1=X1[ID1]
  X2=X2[ID2]
  h1=HISTOGRAM(x1,min=0,max=30,binsize=1,locations=x)
  h2=HISTOGRAM(x2,min=0,max=30,binsize=1,locations=x)

  window,3,xsize=550,ysize=550
